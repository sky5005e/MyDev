...
public class StoreScreen extends MainScreen{
    ...
    private VerticalFieldManager hfm = new VerticalFieldManager();
    ...
    public void StoreScreen(int _flag,int _screen)
    {
    ...
    public void MaternusScreenLayout(final WebDataCallback callback) 
    {
       ...
       try
        ...
        douglas.setFont(DouglasConstants.dfont);
        douglas.setPadding(2,0,2,0);
        innerTable.add(douglas);
        
        EventLabel streetName=new EventLabel(shop.getStreetName()+DouglasConstants.Space+shop.getStreetNumber(),LabelField.NON_FOCUSABLE);
        streetName.setFont(DouglasConstants.mediumfont);
        streetName.setPadding(2,0,2,0);
        innerTable.add(streetName);
        
        EventLabel postalField = new EventLabel(shop.getPostalCode()+DouglasConstants.Space+shop.getTown(),LabelField.NON_FOCUSABLE);
        ...
        BitmapField calimg = new BitmapField(IMG,BitmapField.NON_FOCUSABLE);
        ...
        Font[] fonts = {myFont};
        byte[] attributes = {0};
        ActiveRichTextField telno = new ActiveRichTextField(labelEmailText,offsets, attributes, fonts, null, null,FIELD_LEFT | FOCUSABLE | (long)ActiveRichTextField.USE_TEXT_WIDTH){
    ...
    protected boolean keyChar(char character, int status, int time)
     {  
        ...
        infoinTable.add(telno); 
        infoinTable.setPhoneNo(labelEmailText);
        telno.setChangeListener(itemlistener);
...
//**Add picture from URL
        ...
        outerTable.add(img); 
        distance=flag==1?EventsScreenDetail.store[EventsScreenDetail.selectedIndex].getDistance():PerfumerieScreen.store[PerfumerieScreen.selectedIndex].getDistance();
        ...
        EventLabel distance=new EventLabel(DouglasConstants.ThrLabel+parsedNumber,LabelField.NON_FOCUSABLE);
        distance.setFont(DouglasConstants.smfont);
        mapTable.add(distance);
        groupTable.add(outerTable);
        LabelField spc=new LabelField(DouglasConstants.Space,LabelField.NON_FOCUSABLE);
        groupTable.add(spc);
   ...
   //**Google Map
    ...
    //**Parking Info
         ...
         offnungszeiten.setFont(DouglasConstants.dfont);
         infoTable.add(offnungszeiten);
         EventLabel openingtext=new EventLabel(shop.getRegularOpeningsText(),LabelField.NON_FOCUSABLE);
         ...
         parkingInfoLabel.setFont(DouglasConstants.dfont);
         parkingInfoLabel.setPadding(10,0,0,0);
         infoTable.add(parkingInfoLabel);
         EventLabel parkingInfo=new EventLabel(shop.getParkingInfo(),LabelField.NON_FOCUSABLE);
         parkingInfo.setFont(DouglasConstants.smfont);
         infoTable.add(parkingInfo);
         }
         //infoTable.setMargin(0,0,0,10);
      ...
      route.setChangeListener(routelistener);
 ...
 public void BeautyScreenLayout(final WebDataCallback callback) 
    {
       ...
       try
        ...
        EventLabel teleLabel = new EventLabel(DouglasConstants.Rufen,LabelField.NON_FOCUSABLE);
       ...
       // telno.setPadding(4,0,0,0);
        inTable.add(phnno.ARichTextField(labelEmailText,offsets, attributes, fonts, null, null,FIELD_LEFT | ActiveRichTextField.FOCUSABLE | (long)ActiveRichTextField.USE_TEXT_WIDTH,0)); 
   ...
   public void EventTypeScreenLayout(final WebDataCallback callback) 
        ...
        for(int i=0;i<event.length;i++)
        {
         ...
         eventdetail=vectorToEventArray(eventVector);
              ...
              TableLayoutManager outerTable = new TableLayoutManager(new int[]
            {
             ...
             RichTextLabel descriptionLabel = new RichTextLabel(eventdetail[j].getText(),(DrawStyle.HFULL |  RichTextField.FIELD_VCENTER | RichTextField.NO_NEWLINE | RichTextField.NON_FOCUSABLE));
    ...
    public int getID()
             ...
             {   
                public void fieldChanged(Field field, int context)
                {
                     ...
                     if((field==telno && context==0) || (field==telno && context==1))
                   {
                          ...
                          String phoneNum = telno.getText(); // _phonefield is an EditField
      ...
      FieldChangeListener routelistener = new FieldChangeListener() {   
              public void fieldChanged(Field field, int context)
              {
                 if((field==route && context==0) || (field==route && context==1))
                {
                       UiApplication.getUiApplication().pushScreen(new RouteScreen());   
                }
               }
