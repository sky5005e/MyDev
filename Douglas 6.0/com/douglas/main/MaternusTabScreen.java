 ...
 * Create Tab Screen
 */
public class MaternusTabScreen extends MainScreen 
{
   ...
   public MaternusTabScreen(int _flag,int _screenno,int _screenInfo) {
        flag = _flag;
        _screenno= _screenno;
       ...
       setStatus(vfmStatus);
         ...
         Thread t = new Thread(new Runnable()
        {
                ...
                public void run()
                {
                   MaternusScreen _tts = new MaternusScreen();
                   if(flag==0)
                   {
                    
                    _tts.MaternusScreen(0,0);
                    _tts.MaternusScreenLayout(_contentOne);
                    _tts = new MaternusScreen();
                     int k = screenInfo>0?screenInfo:0;
                    _tts.MaternusScreen(k,0);
                  ...
                  {
                    _tts.MaternusScreen(1,1);
                    _tts.MaternusScreenLayout(_contentOne);
                    _tts = new MaternusScreen();
                     int k = screenInfo>0?screenInfo:0;
                    _tts.MaternusScreen(k,1);
                    _tts.EventTypeScreenLayout(_contentTwo);
                    _tts=new MaternusScreen();
                    _tts.MaternusScreen(1,1);
                    _tts.BeautyScreenLayout(_contentThree);
