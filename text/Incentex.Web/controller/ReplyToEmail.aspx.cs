...
public partial class controller_ReplyToEmail : System.Web.UI.Page
{
    #region Properties
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            //        {
            //            List<Message> lstPOP = FetchAllMessages(Common.POPHost, Common.POPPort, Common.SSL, Common.ReplyTo, Common.ReplyToPassword);
        ...
        {
            if (!Convert.ToBoolean(Application["ReplyToLiveInvalidDate"]))
            {
                if (ex.Message.StartsWith("Invalid Date: Invalid time zone. Input: \""))
                {
                    Application["ReplyToLiveInvalidDate"] = true;
