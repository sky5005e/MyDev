...
$.extend($.validator, {
	...
	classRuleSettings: {
		...
		//added by abhishek
		namnivalue:{namnivalue:true},
		cardexpirydt:{cardexpirydt:true},
		eventdate:{eventdate:true},
		validenddate:{validenddate:true},
		creditcard2: {creditcard2: true},
		isrequired:true,
		validdate:{validdate:true},
		validdatealbum:{validdatealbum:true}
	},
	...
	methods: {
		...
		validzipcode : function( value, element, param ) {
		    // contributed by Devraj Gadhavi on 27th of May, 2013.
		    return this.optional(element) || $.validator.methods.isValidPostalCode(value, param);
		},
...
//           }
		...
		isValidPostalCode : function (postalCode, countryCode) {		    
            switch (countryCode) {
                case "US":
                    postalCodeRegex = /^([0-9]{5})([-\s]{0,3}([0-9]{4}))?$/;
                    break;
                case "CA":
                    postalCodeRegex = /^([ABCEGHJKLMNPRSTVXYabceghjklmnprstvxy][0-9][ABCEGHJKLMNPRSTVWXYZabceghjklmnprstvwxyz])[-|\s]{0,3}([0-9][ABCEGHJKLMNPRSTVWXYZabceghjklmnprstvwxyz][0-9])$/;
                    break;
                default:                    
