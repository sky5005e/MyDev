...
public partial class admin_Company_Employee_RegistrationInfo : PageBase
{
    #region Data Members
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            if (Request.QueryString.Count > 0 && Convert.ToInt64(Request.QueryString.Get("ID")) > 0)
            {
                ...
                ((HyperLink)Master.FindControl("lnkLoginUrl")).NavigateUrl = "~/admin/ViewPendingUsers.aspx";
            }
            else
            {
                Response.Redirect("~/admin/ViewPendingUsers.aspx");
            }
    ...
    protected void ddlCompany_SelectedIndexChanged(object sender, EventArgs e)
    {
        if (!String.IsNullOrEmpty(ddlCompany.SelectedValue) && Convert.ToInt64(ddlCompany.SelectedValue) > 0)
        {
            if (Convert.ToInt64(ddlCompany.SelectedValue) > 0)
            {
                ...
                FillWorkGroupNDepartmentForStore(Convert.ToInt64(ddlCompany.SelectedValue));
            }
        }
        else
        {
    ...
    private void DisplayData()
    {
        try
        {
            ...
            {
                ddlCompany.SelectedValue = objReg.iCompanyName.ToString();
            ...
            hdnRegistrationID.Value = Convert.ToString(objReg.iRegistraionID);
        }
        catch (Exception ex)
        {
            ErrHandler.WriteError(ex);
    ...
    [WebMethod]
    public static String CheckDuplicateEmail(String email, String registrationID)
    {
    ...
    [WebMethod]
    public static String CheckDuplicateEmployeeID(String employeeID, String companyID, String registrationID)
    {
        String methodResponse = String.Empty;
