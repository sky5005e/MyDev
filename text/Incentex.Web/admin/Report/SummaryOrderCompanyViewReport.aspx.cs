...
public partial class admin_Report_SummaryOrderCompanyViewReport : PageBase
{
    #region Data Members
    ...
    IncentexBEDataContext dbContext = new IncentexBEDataContext();
    ...
    protected void GenerateChart()
    {
        ...
        if (objResult != null && objResult.Count > 0)
        {
            ...
            var CompanyResult = objReportRepository.GetCompanyList();

            rpCompany.DataSource = CompanyResult;
            rpCompany.DataBind();
    ...
    protected void BindCompanyOrder(Int64 CompanyID, int ItemIndex)
    {
        ...
        var orderResult = (from result in objResult
                           where result.CompanyID == CompanyID
                           select new
                           {
                               ...
                               AssociateServiceTicketID = result.AssociateServiceTicketID 
                           }).OrderByDescending(o=>o.OrderDate).ToList();


        Repeater rptSummaryOrderView = (Repeater)rpCompany.Items[ItemIndex].FindControl("rptSummaryOrderView");
        rptSummaryOrderView.DataSource = orderResult;
        rptSummaryOrderView.DataBind();

        Label lblCompanyOrderCounter = (Label)rpCompany.Items[ItemIndex].FindControl("lblCompanyOrderCounter");
        lblCompanyOrderCounter.Text = orderResult.Count.ToString();
        ...
        foreach (RepeaterItem repeaterItem in rptSummaryOrderView.Items)
        {
            ...
            HiddenField hdnAssociateTicketID = (HiddenField)repeaterItem.FindControl("hdnAssociateTicketID");
            ...
            try
            {
                ...
                //Get Notest from Serviceticket if its is associated with the order
                if (hdnAssociateTicketID != null)
                {
                    if (!(string.IsNullOrEmpty(hdnAssociateTicketID.Value)))
                    {
                        objList.AddRange(objNotesHistoryRepository.GetNotesForIEPerOrderId(Convert.ToInt64(hdnAssociateTicketID.Value), Incentex.DAL.Common.DAEnums.NoteForType.ServiceTicketIEs, "IEInternalNotes", 0));
                        //db.NoteDetails.Where(le => le.ForeignKey == hdnAssociateTicketID.Value && le.NoteFor == "ServiceTicketCAs" && le.SpecificNoteFor.Equals(null)).OrderByDescending(le => le.ForeignKey).ThenByDescending(le => le.NoteID).ToList();
                    }
                }
                //Get Notest from Serviceticket if its is associated with the order


                foreach (NoteDetail obj in objList.OrderByDescending(x => x.CreateDate).ToList())
                {
            ...
            try
            {
                ...
                foreach (NoteDetail obj in objList.OrderByDescending(n => n.CreateDate).ToList())
                {
            ...
            try
            {
                ...
                if (hdnAssociateTicketID != null)
                {
                    if (!(string.IsNullOrEmpty(hdnAssociateTicketID.Value)))
                    {
                        objList.AddRange(objNotesHistoryRepository.GetNotesSendtoCustomer(Incentex.DAL.Common.DAEnums.NoteForType.ServiceTicketCAs, Convert.ToInt64(hdnAssociateTicketID.Value), ""));
                        //db.NoteDetails.Where(le => le.ForeignKey == hdnAssociateTicketID.Value && le.NoteFor == "ServiceTicketCAs" && le.SpecificNoteFor.Equals(null)).OrderByDescending(le => le.ForeignKey).ThenByDescending(le => le.NoteID).ToList();
                    }
                }
                //Get Notest from Serviceticket if its is associated with the order
    ...
    private void SendNotes(Int64 OrderID, Int64 ToUserInfoID, Int64 NoteType, Int64 AdminUserID, Int64 NoteID, Int64 ReceipientType)
    {
        try
        {
            ...
            if (dsEmailTemplate != null)
            {
                ...
                if (new ManageEmailRepository().CheckEmailAuthentication(objUserInformation.UserInfoID, Convert.ToInt64(Incentex.DAL.Common.DAEnums.ManageEmail.OrderNotes)) == true)
                {
                    ...
                    String ReplyToAddress = sReplyToadd.Substring(0, sReplyToadd.IndexOf('@')) + "+on" + objOrder.OrderID + "un" + objUserInformation.UserInfoID + "ad" + AdminUserID + "cb" + IncentexGlobal.CurrentMember.UserInfoID + "nt" + NoteType + "ni" + NoteID + "rtp" + ReceipientType + "en" + sReplyToadd.Substring(sReplyToadd.IndexOf('@'), sReplyToadd.Length - sReplyToadd.IndexOf('@'));
    ...
    private void SendNotesToCustomer(Int64 OrderID, Order objOrder, Int64 AdminUserID, Int64 NoteID)
    {
        try
        {
            ...
            if (dsEmailTemplate != null)
            {
                ...
                string messagebody = txtCustomerNote.Text.Trim();
                ...
                if (AdminUserID != 0)
                {
                    ...
                    if (new ManageEmailRepository().CheckEmailAuthentication(objAdminUserInformation.UserInfoID, Convert.ToInt64(Incentex.DAL.Common.DAEnums.ManageEmail.OrderNotes)) == true)
                    {
                        ...
                        //rtp: recipient type
                        //en: end of the string
                        String ReplyToAddress = sReplyToadd.Substring(0, sReplyToadd.IndexOf('@')) + "+on" + objOrder.OrderID + "un" + objUserInformation.UserInfoID + "ad" + AdminUserID + "cb" + IncentexGlobal.CurrentMember.UserInfoID + "nt3ni" + NoteID + "rtp0en" + sReplyToadd.Substring(sReplyToadd.IndexOf('@'), sReplyToadd.Length - sReplyToadd.IndexOf('@'));
                        //if needs to insert in the todays emails send history than just pass adminuserid instead of 0 in the below line
                        new CommonMails().SendMailWithReplyTo(0, "Order Notes", sFrmadd, objAdminUserInformation.Email, sSubject, messagebody.ToString(), sFrmname, ReplyToAddress, Common.UserName, Common.Password, Common.SMTPHost, Common.SMTPPort.ToString(), "Normal", Common.SSL, true, OrderID);
                    }
                }
                ...
                if (new ManageEmailRepository().CheckEmailAuthentication(objUserInformation.UserInfoID, Convert.ToInt64(Incentex.DAL.Common.DAEnums.ManageEmail.OrderNotes)) == true)
                {
                    ...
                    //rtp: recipient type
                    //en: end of the string
                    String ReplyToAddress = sReplyToadd.Substring(0, sReplyToadd.IndexOf('@')) + "+on" + objOrder.OrderID + "un" + objUserInformation.UserInfoID + "ad" + AdminUserID + "cb" + IncentexGlobal.CurrentMember.UserInfoID + "nt3ni" + NoteID + "rtp" + objUserInformation.Usertype + "en" + sReplyToadd.Substring(sReplyToadd.IndexOf('@'), sReplyToadd.Length - sReplyToadd.IndexOf('@'));
                    new CommonMails().SendMailWithReplyTo(objUserInformation.UserInfoID, "Order Notes", sFrmadd, sToadd, sSubject, messagebody.ToString(), sFrmname, ReplyToAddress, Common.UserName, Common.Password, Common.SMTPHost, Common.SMTPPort.ToString(), "Normal", Common.SSL, true, OrderID);
                }
            }
        }
    ...
    protected void rpCompany_ItemDataBound(object sender, RepeaterItemEventArgs e)
    {
    ...
    protected void lnkSelect_Click(object sender, EventArgs e)
    {
        ...
        Int64 CompanyID = Convert.ToInt64(lnkSelect.CommandArgument);
        hfTempCompanyID.Value = Convert.ToString(CompanyID);
        hdnMainRepeaterItemIndex.Value = Convert.ToString(rpCust.ItemIndex);
        BindCompanyOrder(CompanyID, Convert.ToInt32(hdnMainRepeaterItemIndex.Value));
    }
    ...
    protected void lnkbtnSubmitNote_Click(object sender, EventArgs e)
    {
        if (!string.IsNullOrEmpty(hdnNoteType.Value) && !string.IsNullOrEmpty(hdnOrderID.Value))
        {
                ...
                if (chkIsEmailSend.Checked)
                {
                    foreach (DataListItem item in dtIE.Items)
                    {
                        if (((CheckBox)item.FindControl("chkUser")).Checked == true)
                        {
                            ...
                            objOrderRep.InsertIEListIntoOrderNoteRecipient(OrderID, objNoteDetail.NoteID, Convert.ToInt64(hdnUserID.Value), IncentexGlobal.CurrentMember.UserInfoID);
                            SendNotes(OrderID, Convert.ToInt64(hdnUserID.Value), 1, 0, objNoteDetail.NoteID, Convert.ToInt64(Incentex.DAL.Common.DAEnums.UserTypes.IncentexAdmin));
                            //SendNotes(OrderID, 1092, 2);
                        }
                    }

                    SendNotes(OrderID, objSupplierRepository.GetById(Convert.ToInt64(ddlVendor.SelectedValue)).UserInfoID, 1, 0, objNoteDetail.NoteID, Convert.ToInt64(Incentex.DAL.Common.DAEnums.UserTypes.Supplier));
                ...
                if (chkIsEmailSend.Checked)
                {
                    foreach (DataListItem item in dtIE.Items)
                    {
                        if (((CheckBox)item.FindControl("chkUser")).Checked == true)
                        {
                            HiddenField hdnUserID = ((HiddenField)item.FindControl("hdnUserID"));
                            objOrderRep.InsertIEListIntoOrderNoteRecipient(OrderID, objNoteDetail.NoteID, Convert.ToInt64(hdnUserID.Value), IncentexGlobal.CurrentMember.UserInfoID);
                            SendNotes(OrderID, Convert.ToInt64(hdnUserID.Value), 2, 0, objNoteDetail.NoteID, Convert.ToInt64(Incentex.DAL.Common.DAEnums.UserTypes.IncentexAdmin));
                            //SendNotes(OrderID, 1092, 2);
                        }
                    }
                }
            ...
            else if (NoteType == "Send Customer Message" && txtCustomerNote.Text.Trim() != "")
            {
                ...
                {
                    foreach (DataListItem item in dtIncentexEmployee.Items)
                    {
                        if (((CheckBox)item.FindControl("chkUser")).Checked == true)
                        {
                            HiddenField hdnUserID = ((HiddenField)item.FindControl("hdnUserID"));
                            objOrderRep.InsertIEListIntoOrderNoteRecipient(OrderID, objNoteDetail.NoteID, Convert.ToInt64(hdnUserID.Value), IncentexGlobal.CurrentMember.UserInfoID);
                            SendNotes(OrderID, Convert.ToInt64(hdnUserID.Value), 3, Convert.ToInt64(ddlAdminForWorkgroup.SelectedValue), objNoteDetail.NoteID, Convert.ToInt64(Incentex.DAL.Common.DAEnums.UserTypes.IncentexAdmin));
                            //SendNotes(OrderID, 1092, 2);
                        }
                    }
                        
                    SendNotesToCustomer(OrderID, objOrder, Convert.ToInt64(ddlAdminForWorkgroup.SelectedValue), objNoteDetail.NoteID);
                }
                //added by Prashant on 20th nov 2012
            }
        }
        GenerateChart();
        BindCompanyOrder(Convert.ToInt64(hfTempCompanyID.Value),Convert.ToInt32(hdnMainRepeaterItemIndex.Value));
    }
    ...
    protected void rptSummaryOrderView_ItemCommand(object source, RepeaterCommandEventArgs e)
    {
        try
        {
            ...
            {
                dtIncentexEmployee.DataSource = new IncentexEmployeeRepository().GetAllEmployee();
                dtIncentexEmployee.DataBind();
