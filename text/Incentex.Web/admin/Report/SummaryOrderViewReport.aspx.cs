...
public partial class admin_Report_SummaryOrderViewReport : PageBase
{
    #region Data Members
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            if (!string.IsNullOrEmpty(Request.QueryString["OrderNumber"]))
                txtOrderNumber.Text = Request.QueryString["OrderNumber"];
            ...
            if (!string.IsNullOrEmpty(Request.QueryString["StatusView"]))
                ddlStatusView.SelectedValue = Request.QueryString["StatusView"];
    ...
    protected void rptSummaryOrderView_ItemCommand(object source, RepeaterCommandEventArgs e)
    {
        try
        {
            ...
            else if (e.CommandName == "Send Customer Message")
            {
                ...
                hfWorkGroupID = (HiddenField)rptSummaryOrderView.Items[e.Item.ItemIndex].FindControl("hfWorkGroupID");
                HiddenField hfCompanyID = new HiddenField();
                hfCompanyID = (HiddenField)rptSummaryOrderView.Items[e.Item.ItemIndex].FindControl("hfCompanyID");
    ...
    protected void GenerateChart()
    {
        ...
        if (ddlStatusView.SelectedValue != "")
            StatusView = ddlStatusView.SelectedValue;
        ...
        List<SelectOrderSummaryViewReportResult> objResult = objReportRepository.GetOrderSummaryViewReport(UserInfoID, FromDate, ToDate, StoreID, WorkgroupID, BaseStationID, GenderID, OrderNumber, FirstName, LastName, StatusView);
        ...
        if (objResult != null && objResult.Count > 0)
        {
            ...
            lblSummaryOrderViewRecordCounter.Text = orderResult.Count.ToString();
        ...
        try
        {
            ...
            if (dsEmailTemplate != null)
            {
                ...
                if (new ManageEmailRepository().CheckEmailAuthentication(objUserInformation.UserInfoID, Convert.ToInt64(Incentex.DAL.Common.DAEnums.ManageEmail.OrderNotes)) == true)
                {
                    ...
                    //rtp: recipient type
                    //en: end of the string
                    String ReplyToAddress = sReplyToadd.Substring(0, sReplyToadd.IndexOf('@')) + "+on" + objOrder.OrderID + "un" + objUserInformation.UserInfoID + "ad" + AdminUserID + "cb" + IncentexGlobal.CurrentMember.UserInfoID + "nt3ni" + NoteID + "rtp" + "rtp" + objUserInformation.Usertype + "en" + sReplyToadd.Substring(sReplyToadd.IndexOf('@'), sReplyToadd.Length - sReplyToadd.IndexOf('@'));
                    new CommonMails().SendMailWithReplyTo(objUserInformation.UserInfoID, "Order Notes", sFrmadd, sToadd, sSubject, messagebody.ToString(), sFrmname, ReplyToAddress, Common.UserName, Common.Password, Common.SMTPHost, Common.SMTPPort.ToString(), "Normal", Common.SSL, true, OrderID);
                }
            }
        }
