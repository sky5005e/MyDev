...
/// Description : This page is for approve/cancel moas order comming from email confirmation of CA
/// Created : Mayur on 16-nov-2011
/// </summary>
...
public partial class MOASOrderStatus : System.Web.UI.Page
{
    #region Data Members
    ...
    protected void Page_Load(Object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            if (!String.IsNullOrEmpty(Request.QueryString["UserId"]) && !String.IsNullOrEmpty(Request.QueryString["OrderId"]) && !String.IsNullOrEmpty(Request.QueryString["Status"]))
            {
                Order objOrder = objRepos.GetByOrderID(Convert.ToInt64(Request.QueryString["OrderId"]));

                if (objOrder != null)
                {
                    //Update by Prashant May 2013
                    //return if Ordestatus is not Waiting For Review
                    if (objOrder.OrderStatus.ToUpper() != "ORDER PENDING" && objOrder.OrderStatus.ToUpper() != "CANCELED")
                    {
                        ((Label)Master.FindControl("lblPageHeading")).Text = "Already Approved";
                        dvOrderAlreadyApproved.Visible = true;
                        dvOrderCancel.Visible = false;
                        dvOrderSuccessfullyApproved.Visible = false;
                        dvOrderSuccessfullyCancel.Visible = false;
                        dvOrderAlreadyCanceled.Visible = false;
                        return;
                    }
                    else if (objOrder.OrderStatus.ToUpper() == "CANCELED")
                    {
                        ((Label)Master.FindControl("lblPageHeading")).Text = "Already Cancelled";
                        dvOrderAlreadyApproved.Visible = false;
                        dvOrderCancel.Visible = false;
                        dvOrderSuccessfullyApproved.Visible = false;
                        dvOrderSuccessfullyCancel.Visible = false;
                        dvOrderAlreadyCanceled.Visible = true;
                        return;
                    }
                    //Update by Prashant May 2013
                    ...
                    this.MOASUserID = Convert.ToInt64(Request.QueryString["UserId"]);
                    OrderMOASSystem objOrderMOASSystem = objOrderMOASSystemRepository.GetByOrderIDAndManagerUserInfoID(this.OrderID, this.MOASUserID);

                    //if order is already been changed by other MOAS user then not update again.
                    if (objOrder.OrderStatus.ToUpper() != "ORDER PENDING" || objOrderMOASSystem == null || (objOrderMOASSystem != null && objOrderMOASSystem.Status.ToUpper() != "ORDER PENDING"))
                    {
                        ((Label)Master.FindControl("lblPageHeading")).Text = "Already Approved";
                        dvOrderAlreadyApproved.Visible = true;
                        dvOrderCancel.Visible = false;
                    ...
                    {
                        ((Label)Master.FindControl("lblPageHeading")).Text = "Reason for Cancelling";
                        dvOrderAlreadyApproved.Visible = false;
                        dvOrderCancel.Visible = true;
                        dvOrderSuccessfullyApproved.Visible = false;
                        dvOrderAlreadyCanceled.Visible = false;
                        dvOrderSuccessfullyCancel.Visible = false;
                    }
                    else if (Request.QueryString["Status"] == "Approve")
                    {
                        this.IsMOASWithCostCenterCode = objCompanyEmployeeRepository.FUN_IsMOASWithCostCenterCode(Convert.ToInt64(objOrder.UserId));

                        ((Label)Master.FindControl("lblPageHeading")).Text = "Officially Approved";

                        //change start for priority wise sending email 3-feb-2012 by mayur
                        ...
                        if (!String.IsNullOrEmpty(ApproverLevel) && ApproverLevel.ToLower() == "companylevel")
                        {
                            ...
                            List<OrderMOASSystem> objOrderMOASSystemList = objOrderMOASSystemRepository.GetByOrderIDAndStatus(this.OrderID, "Order Pending");

                            if (objOrderMOASSystemList != null && objOrderMOASSystemList.Count > 0)
                            {
                                AddNoteHistory("Approved");
                                ...
                                ////Check Email Authentication and Send Mail to appropriate Admin
                                ...
                                //    Common.EmailNotificationDetails mailBody = Common.GetEmailMessageBodyForMOASManager(OrderID, false);

                                //    if (mailBody != null)
                                //    {
                                //        String smtphost = CommonMails.SMTPHost;
                                //        Int32 smtpport = CommonMails.SMTPPort;
                                //        String MessBody = mailBody.MessageBody.Replace("{FullName}", objUserInformation.FirstName + " " + objUserInformation.LastName);
                                //        MessBody = MessBody.Replace("{MOASUserId}", Convert.ToString(objUserInformation.UserInfoID));
                                //        new CommonMails().SendMail(objUserInformation.UserInfoID, "MOAS From Email", mailBody.sFromAddress, objUserInformation.LoginEmail, mailBody.sSubject, MessBody.ToString(), mailBody.sFromName, smtphost, smtpport, false, true, objOrder.OrderID);
                                //    }
                                //}   

                                sendVerificationEmailMOASManager(this.OrderID, objUserInformation.LoginEmail, objUserInformation.FirstName, objUserInformation.UserInfoID);
                            }
                            else
                            {
                                UpdateOrderAndSendConfirmation(objOrder, objUser);
                            }
                            //change end for priority wise sending email 3-feb-2012 by mayur
                        }
                        else
                        {
                        ...
                        dvOrderCancel.Visible = false;
                        dvOrderAlreadyCanceled.Visible = false;
                        dvOrderSuccessfullyApproved.Visible = true;
                        dvOrderSuccessfullyCancel.Visible = false;
                    }
                }
            }
    ...
    {
        //updated on 26th Feb 2013 by prashant
    ...
    {
        Order objOrder = objRepos.GetByOrderID(this.OrderID);
        if (objOrder != null)
        {
            UserInformation objUser = objUsrInfoRepo.GetById(Convert.ToInt64(objOrder.UserId));
            ...
            if (objOrder.CreditUsed != null)
            {
                ...
                CompanyEmployee cmpEmpl = objCmnyEmp.GetByUserInfoId((Int64)objOrder.UserId);
                if (objOrder.CreditUsed == "Previous")
                {
                    cmpEmpl.StratingCreditAmount = cmpEmpl.StratingCreditAmount + objOrder.CreditAmt;
                    objCmnyEmp.SubmitChanges();
                }
                else if (objOrder.CreditUsed == "Anniversary")
                {
                    cmpEmpl.CreditAmtToApplied = cmpEmpl.CreditAmtToApplied + objOrder.CreditAmt;
                    objCmnyEmp.SubmitChanges();
                }
                else
                {
            ...
            List<Order> obj = objRepos.GetShoppingCartId(objOrder.OrderNumber);
            if (obj.Count > 0)
            {
                String[] a;
                ...
                foreach (String u in a)
                {
                    if (objOrder.OrderFor == "ShoppingCart")
                    {
                        ...
                        objShoppingcart = objShoppingCartRepository.GetById(Convert.ToInt32(u), (Int64)objOrder.UserId);
                    ...
                    else
                    {
                        ...
                        objIssuance = objIssuanceRepos.GetById(Convert.ToInt64(u), (Int64)objOrder.UserId);
                        List<SelectProductIDResult> objList = objProItemRepos.GetProductId(objIssuance.MasterItemID, Convert.ToInt64(u), Convert.ToInt64(objIssuance.ItemSizeID), Convert.ToInt64(objOrder.StoreID));
                        //Update Inventory Here 
                        //Call here upDate Procedure
            ...
            sendVerificationEmail("Canceled", objOrder.OrderID, objUser.LoginEmail, objUser.FirstName); //send email to CE

            if (!objOrder.ReferenceName.ToLower().Contains("test") && !IncentexGlobal.IsIEFromStoreTestMode)
                sendIEEmail("Canceled", objOrder.OrderID);//send email to IE
            else
                sendTestOrderEmailNotification("Canceled", objOrder.OrderID);

            ((Label)Master.FindControl("lblPageHeading")).Text = "Officially Cancelled";
            dvOrderAlreadyApproved.Visible = false;
            dvOrderAlreadyCanceled.Visible = false;
            dvOrderCancel.Visible = false;
            dvOrderSuccessfullyApproved.Visible = false;
            dvOrderSuccessfullyCancel.Visible = true;
        }
    }
    #endregion
    ...
    private void sendVerificationEmail(String strStatus, Int64 OrderID, String ToAdd, String FullName)
    {
        try
        {
            ...
            if (dsEmailTemplate != null)
            {
                ...
                messagebody.Replace("{OrderTotal}", "$" + Convert.ToDecimal(objOrder.ShippingAmount + (objOrder.MOASSalesTax != null && objOrder.MOASSalesTax != 0 ? Convert.ToDecimal(objOrder.MOASSalesTax) : Convert.ToDecimal(objOrder.SalesTax)) + Convert.ToDecimal(objOrder.OrderAmount)).ToString());
                ...
                if (new ManageEmailRepository().CheckEmailAuthentication(objUserInformation.UserInfoID, Convert.ToInt64(Incentex.DAL.Common.DAEnums.ManageEmail.MOAS)) == true)
                {
                    new CommonMails().SendMail(objUserInformation.UserInfoID, "MOAS From Email", sFrmadd, sToadd, sSubject, messagebody.ToString(), sFrmname, smtphost, smtpport, false, true, objOrder.OrderID);
                }
            }
        }
        catch (Exception ex)
    ...
    private void sendTestOrderEmailNotification(String strStatus, Int64 OrderID)
    {
        ...
        if (objAdminList.Count > 0)
        {
            ...
            {
                sendVerificationEmailAdmin(strStatus, OrderID, receiver.LoginEmail, receiver.FirstName, Convert.ToInt64(receiver.UserInfoID), true);
            }
        }
    }
    ...
    private void sendVerificationEmailMOASManager(Int64 OrderId, String MOASEmailAddress, String FullName, Int64 MOASUserId)
    {
        try
        {
            ...
            if (dsEmailTemplate != null)
            {
                ...
                messagebody.Replace("{ShippingCost}", objOrder.ShippingAmount.ToString());
                messagebody.Replace("{Saletax}", "$" + (objOrder.MOASSalesTax != null && objOrder.MOASSalesTax != 0 ? Convert.ToDecimal(objOrder.MOASSalesTax) : Convert.ToDecimal(objOrder.SalesTax)).ToString());
                messagebody.Replace("{OrderTotal}", "$" + Convert.ToDecimal(objOrder.ShippingAmount + (objOrder.MOASSalesTax != null && objOrder.MOASSalesTax != 0 ? Convert.ToDecimal(objOrder.MOASSalesTax) : Convert.ToDecimal(objOrder.SalesTax)) + (objOrder.MOASOrderAmount != null ? objOrder.MOASOrderAmount : objOrder.OrderAmount)).ToString());
                if (objOrder.CorporateDiscount != null && objOrder.CorporateDiscount != 0)//check that corporate discount is applied or not
                {
                    messagebody.Replace("{CorporateDiscountView}", "Corporate Discount");
                    messagebody.Replace("{CorporateDiscount}", objOrder.CorporateDiscount.ToString());
                ...
                {
                    new CommonMails().SendMail(MOASUserId, "MOAS From Email", sFrmadd, sToadd, sSubject, messagebody.ToString(), sFrmname, smtphost, smtpport, false, true, objOrder.OrderID);
                }
            }
    ...
    protected void AddNoteHistory(String strStatus)
    {
        ...
        objComNot.CreatedBy = this.MOASUserID;
        objComNot.UpdateDate = System.DateTime.Now;
        objComNot.UpdatedBy = this.MOASUserID;
