...
public partial class usercontrol_OpenServiceticketCA : System.Web.UI.UserControl
{
    ...
    protected void btnSubmitTicketCA_Click(object sender, EventArgs e)
    {
        try
        {
            if (Convert.ToBoolean(Session["ostCAControl"]))
            {
                Session.Remove("ostCAControl");
                if (IncentexGlobal.CurrentMember != null)
                {
                    ...
                    ServiceTicketID = objSerTicRep.InsertTicket(IncentexGlobal.CurrentMember.UserInfoID, IncentexGlobal.CurrentMember.CompanyId, IncentexGlobal.CurrentMember.UserInfoID, null, OwnerID, DateTime.Now.ToString("MM/dd/yyyy"), Convert.ToString(txtSubject.Text.Trim()), Convert.ToString(txtQuestion.Text.Trim()));
                    ...
                    UserInformation objOwner = objUserRepo.GetById(Convert.ToInt64(ddlServiceTicketOwner.SelectedValue));
    ...
    private void FillServiceTicketOwner()
    {
        try
        {
            ...
            var lstServiceTicketOwners = objRepo.GetServiceTicketOwnersForCA(IncentexGlobal.CurrentMember.UserInfoID).Where(le => le.Existing == 1).Select(le => new { UserInfoID = le.UserInfoID, EmployeeName = le.FirstName + " " + le.LastName }).OrderBy(le => le.EmployeeName).ToList();

            ddlServiceTicketOwner.DataSource = lstServiceTicketOwners;
