...
public partial class MyAccount_SearchTicketResult : PageBase
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            ((HyperLink)Master.FindControl("lnkLoginUrl")).NavigateUrl = "~/MyAccount//SearchTicketCA.aspx";

            if (!String.IsNullOrEmpty(Convert.ToString(Request.QueryString)))
            {
                if (!String.IsNullOrEmpty(Request.QueryString["wg"]))
                    this.WorkGroupID = Convert.ToInt32(Request.QueryString["wg"]);
                else
    ...
    protected void lnkClose_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            new ServiceTicketRepository().UpdateNoteReadFlag(TicketID, IncentexGlobal.CurrentMember.UserInfoID, new ServiceTicketRepository().GetUnreadNotes(TicketID, IncentexGlobal.CurrentMember.UserInfoID).ToList());
    ...
    private void BindRepeater()
    {
        try
        {
            ...
            String AllowedToSearch = new PreferenceRepository().GetUserPreferenceValue(IncentexGlobal.CurrentMember.UserInfoID, IncentexGlobal.CurrentMember.Usertype, "SearchTicketsOpt");

            this.lstTickets = new ServiceTicketRepository().SelectServiceTicketsBySearchCriteriaForCA(IncentexGlobal.CurrentMember.UserInfoID, this.TicketName, this.TicketNumber, this.WorkGroupID, this.ContactID, this.StatusID, this.OwnerID, this.KeyWord, AllowedToSearch);
