...
namespace Itstep.Jobmachine.Crawler.Core.Providers
{
    public class PsiProvider
    {
         ...
         private string _url = @"http://www.psi.ch/pa/offenestellen/";// This is the Acctual URL
        ...
        private void ParseHtml(string html, IList<JobAdDto> jobAds)
        {
            try
            {
                ...
                var parsedValues = _htmlDoc.DocumentNode.SelectNodes("//table[@id='jobs']/tbody/tr");// 
                ...
                foreach (var item in parsedValues)
                {
                    ...
                    Region = item.SelectNodes(".//td")[tdcount - 2].InnerText.Trim();

                    jobAds.Add(new JobAdDto() { Description = Description, Source = new Uri(string.Format("http://www.psi.ch/{0}", Link)), Region = Region });
                }
