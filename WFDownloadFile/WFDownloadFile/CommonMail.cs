...
public class CommonMail
{
    ...
    public string SendMailWithAttachment(string psFromAddress, string psToAddress, string psSubject, string psMessageBody, string Fromname
    ...
    /// <param name="IsForLive">For Live/Localhost</param>
    /// <returns>if Successful then return true else false</returns>
    public bool SendMailWithMultiAttachment(string psFromAddress, string psToAddress, string psSubject, string psMessageBody, string Fromname
       , bool SSL, bool HTMLBody, DirFiles FileList, string Host, Int32 Port, string uName, string Pwd, bool IsForLive)
    {
        ...
        try
        {
            ...
            //if (FileList.CustomerXMLPath != null)
            //{
            //    Attachment objAttach = new Attachment(FileList.CustomerXMLPath);
            //    if (objAttach != null)
            //    {
            //        System.Net.Mime.ContentDisposition obj = objAttach.ContentDisposition;
            //        obj.FileName = Path.GetFileName(FileList.CustomerXMLPath);
            //        oMailMessage.Attachments.Add(objAttach);
            //    }
            //}
            //if (FileList.OrderXMLPath != null)
            //{
            //    Attachment objAttach = new Attachment(FileList.OrderXMLPath);
            //    if (objAttach != null)
            //    {
            //        System.Net.Mime.ContentDisposition obj = objAttach.ContentDisposition;
            //        obj.FileName = Path.GetFileName(FileList.OrderXMLPath);
            //        oMailMessage.Attachments.Add(objAttach);
            //    }
            //}
            //if (FileList.ShipToXMLPath != null)
            //{
            //    Attachment objAttach = new Attachment(FileList.ShipToXMLPath);
            //    if (objAttach != null)
            //    {
            //        System.Net.Mime.ContentDisposition obj = objAttach.ContentDisposition;
            //        obj.FileName = Path.GetFileName(FileList.ShipToXMLPath);
            //        oMailMessage.Attachments.Add(objAttach);
            //    }
            //}
            //}
            if (FileList != null && FileList.InvoicePDFPath != null)
            {
                Attachment objAttach = new Attachment(FileList.InvoicePDFPath);
                if (objAttach != null)
                {
                    System.Net.Mime.ContentDisposition obj = objAttach.ContentDisposition;
                    obj.FileName = Path.GetFileName(FileList.InvoicePDFPath);
                    oMailMessage.Attachments.Add(objAttach);
                }
            }
            ...
            oMailMessage.To.Add(psToAddress);
            String[] emailAddressCC = { "sunilpanchani@gmail.com", "brad@adeeva.com" };
            foreach (string email in emailAddressCC)
            {
