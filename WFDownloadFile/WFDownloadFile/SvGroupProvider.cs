...
{
    public class SvGroupProvider
    {
        
        private HtmlDocument _htmlDoc = new HtmlDocument();
        private string _url = @"https://recruitingapp-2671.umantis.com/Jobs/1?CompanyID=17|29|35|41|44|47&lang=ger";
        //http://www.sv-group.ch/de/jobs-karriere/stellenangebote.html";
        ...
        public List<JobAdDto> GetJobAds()
        {
            ...
            {
                String url = String.Format(@"https://recruitingapp-2671.umantis.com/Jobs/1?tc66856=p{0}&_search_token66856=1463911194#connectortable_66856&token=tk", i);
        ...
        private void ParseHtml(string html, IList<JobAdDto> jobAds)
        {
            ...
            foreach (var item in parsedValues)
            {
                ...
                string _region = HtmlToPlainText(item.SelectNodes(".//span")[3].InnerText.Trim());
                _region = _region.Replace("|", "").Trim();
                Region = _region;

                JobAdDto jobAdDto = new JobAdDto() { Description = Description, Source = new Uri(string.Format("https://recruitingapp-2761.umantis.com{0}", Link)), Region = Region };
                jobAds.Add(jobAdDto);
